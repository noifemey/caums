      ...
      <CCard>
        ...
        <CCardBody>
          ...
          <div class="row">
            ...
            <div class="col-2">
              <label></label>
              <button type="button" class="btn btn-primary btn-lg btn-block" @click = "exportRCI"> Export </button>
            </div>
          </div>
          </form>
...
export default {
  ...
  data: () => {
    return {
      ...
      grandTotal:"",
      table : {
        columns: ['DateIssued', 'CheckNo','VoucherNo','CenterCode','Payee','Purpose','CkAmount'],
        items: [],
        options: {
            headings: {
              DateIssued: 'Check Date',
              CheckNo: 'Check Number',
              VoucherNo: 'DV No./Payroll',
              CenterCode: 'Responsibility Center Code',
              Payee: 'Payee',
              Purpose: 'Nature of Payment',
              CkAmount: 'Amount'
            },                    
            rowClassCallback: function(row) { 
                return (row.month_name == "Grand Total") ?'bg-warning':'';
            },
            filterable: ['DateIssued','CheckNo','VoucherNo','Payee','Purpose'],
        }
      }
    }
  },
  methods: {      
    ...
    getData (){
      ...
      axios.get(  this.$apiAdress + '/api/reports/get-rci?token=' + localStorage.getItem("api_token"),
        { params : {
    ...
    exportRCI(){
      ...
      axios.post(  
        this.$apiAdress + '/api/reports/export-rci?token=' + localStorage.getItem("api_token"),
          params, {
          responseType: 'blob'
        }).then(function (response) {
          ...
          var today = new Date();
          link.setAttribute('download', 'RCI-Report_' + today +'.xlsx');
